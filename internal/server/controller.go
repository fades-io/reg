...
func (server *Server) Reg(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		return apperror.NewAppError(err, logs.InvalidRequestBody, err.Error(), http.StatusUnprocessableEntity)
	}
	...
	if err != nil {
		return apperror.NewAppError(err, logs.JsonParsingError, err.Error(), http.StatusUnprocessableEntity)
	}

	user.Prepare()
	err = user.Validate("reg")
	if err != nil {
		return apperror.NewAppError(err, logs.InvalidFormat, err.Error(), http.StatusBadRequest)
	}
